...
internal static class Program
{
        ...
        try
        {
                ...
                string.IsNullOrEmpty(installDir) || string.IsNullOrEmpty(launchAfter))
            {
                Console.Error.WriteLine(
                    "Usage: SwellSSH.Updater --parent-pid=N --extracted-dir=PATH --install-dir=PATH --launch-after=NAME [--elevated]");
                return 2;
            }
    ...
    private static bool TryEnsureWritable(string installDir)
    {
        try
        {
            var probe = Path.Combine(installDir, ".swellssh-write-test");
    ...
    private static void RelaunchElevated(string[] originalArgs)
    {
        ...
        {
            FileName        = Environment.ProcessPath ?? "SwellSSH.Updater.exe",
    ...
    private static void CleanupLargeStagingDirs(string extractedDir)
    {
        ...
        if (string.IsNullOrEmpty(stageRoot)) return;
